...
void digest_message(const unsigned char *message, size_t message_len, unsigned char **digest, unsigned int *digest_len) {
	...
	if(1 != EVP_DigestInit_ex(mdctx, EVP_sha256(), NULL)) return;

	if(1 != EVP_DigestUpdate(mdctx, message, message_len)) return;

	if((*digest = (unsigned char *)OPENSSL_malloc(EVP_MD_size(EVP_sha256()))) == NULL) return;
...
int generate(char *fichier, char *fichier_output) {
	...
	while(fgets(ligne, sizeof(ligne), fp)) {
		...
		digest_message((unsigned char*)ligne, strlen(ligne), &digest, &digest_len);
		if(!digest) {
			fclose(fp);
			fclose(fo);
			return 1;
